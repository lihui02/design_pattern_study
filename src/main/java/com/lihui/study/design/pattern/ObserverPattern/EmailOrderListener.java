 ...
 */
@Component
public class EmailOrderListener implements ApplicationListener<OrderAddEvent> {
    public void onApplicationEvent(OrderAddEvent orderEvent) {
        Order order = (Order) orderEvent.getSource();
        System.out.println("发送邮件,用户:"+order.getUsername()+"你的订单编号为:"+order.getCode()+"价格"+order.getPrice());
    }
}
